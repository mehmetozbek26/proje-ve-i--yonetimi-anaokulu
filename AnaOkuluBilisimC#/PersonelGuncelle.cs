    ...
    public partial class PersonelGuncelle : Form
    {
            ...
            try
            {
                ...
                for (int i = 0; i < cmbDepartman.Items.Count; i++)
                    if (cmbDepartman.Items[i].ToString() == prskayit.dataGridView1.CurrentRow.Cells["Departman"].Value.ToString())
                    {
                        ...
                        departmanogr = prskayit.dataGridView1.CurrentRow.Cells["Departman"].Value.ToString();
                        break;
                    }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (departmanogr.Equals("OGRETMEN") && !departmanogr.Equals(cmbDepartman.Items[cmbDepartman.SelectedIndex].ToString()) ) throw new Exception("Ogremenlerin Departman Bölümü Değiştirilemez");
                ...
                {
                    using (SqlCommand cmd1 = new SqlCommand("SELECT * FROM Ogretmenler where PersonelID=" + id, cnn))
                    {
                        if (cnn.State != ConnectionState.Open)
                            cnn.Open();
                        ...
                        if (rd.Read())
                        {
                            if (!rd.IsClosed)
                                rd.Close();
                            using (SqlCommand cmd2 = new SqlCommand("Update Ogretmenler set Adi='" + txtAd.Text + "'," + "SoyAdi='" + txtSoyad.Text + "' where PersonelID=" + id, cnn))
                            {
                                if (cnn.State != ConnectionState.Open)
                                    cnn.Open();
                        ...
                        else
                        {
                            if (!rd.IsClosed)
                                rd.Close();
                            using (SqlCommand cmd3 = new SqlCommand("INSERT INTO Ogretmenler (PersonelID,Adi,Soyadi) VALUES (" + id + ",'" + txtAd.Text + "','" + txtSoyad.Text + "')", cnn))
                            {
                                if (cnn.State != ConnectionState.Open)
                                    cnn.Open();
